 ...
 *
 * This is a fork of Groovysh that allows us to customize the command prompt and output messages.  there is an issue
 * open to fix this class http://jira.codehaus.org/browse/GROOVY-2832. When it gets closed we should be able to remove this.
 */
class IBeansGroovysh extends Shell
{
  private static final MessageSource messages = new MessageSource(IBeansGroovysh.class)
  ...
  IBeansGroovysh(final ClassLoader classLoader, final Binding binding, final IO io, final Closure registrar)
  {
    super(io)
  ...
  IBeansGroovysh(final ClassLoader classLoader, final Binding binding, final IO io)
  {
    this(classLoader, binding, io, createDefaultRegistrar())
  }

  IBeansGroovysh(final Binding binding, final IO io)
  {
    this(Thread.currentThread().contextClassLoader, binding, io)
  }

  IBeansGroovysh(final IO io)
  {
    this(new Binding(), io)
  }
    ...
    try
    {
      ...
      else
      {
        ...
        addShutdownHook {
          ...
          io.out.println(messages.format("shutdown", IBeansInfo.getProductName()))
          try
          {
            MuleContext context = this.interp.getContext().getVariable("muleContext")
            context.dispose()
          }
          catch (Exception e)
          {
            io.out.println(messages.format("shutdown", e.getMessage() + "(" + e.class.name + ")"))
          }
        }
        ...
        if (!io.quiet)
        {
          ...
          io.out.println(messages.format('startup_banner.0', IBeansInfo.getProductName(), IBeansInfo.getProductVersion(), System.properties['java.version']))
          io.out.println(messages.format('startup_banner.1', IBeansInfo.getProductUrl()))
